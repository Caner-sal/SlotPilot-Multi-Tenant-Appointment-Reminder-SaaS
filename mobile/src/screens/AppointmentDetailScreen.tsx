...
export default function AppointmentDetailScreen({ appointmentId, token, onBack }: Props) {
  ...
  useEffect(() => {
    apiFetch<{ data: Appointment }>(`/api/appointments/${appointmentId}`, {}, token)
      ...
      .catch(() => Alert.alert("Error", "Could not load appointment."))
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.card}>
        ...
        <Row
          label="Start"
          value={new Date(appointment.startTime).toLocaleString([], { dateStyle: "medium", timeStyle: "short" })}
        />
        <Row
          label="End"
          value={new Date(appointment.endTime).toLocaleString([], { dateStyle: "medium", timeStyle: "short" })}
        />
      </View>
        ...
        <View style={styles.actions}>
          ...
          {transitions.map((s) => (
            <TouchableOpacity
              ...
              disabled={updating}
            >
              <Text style={styles.actionButtonText}>{s}</Text>
            </TouchableOpacity>
          ))}
        </View>
      )}
